...
def process_video(input_path, json_path, pdf_path, store_capacity=50, conf_thresh=0.3, iou_thresh=0.45):
    ...
    print("\n========== STAGE 3: SKU PRODUCT DETECTOR MODEL (best.pt) ==========")
    ...
    # ----------------------------------------------------
    shelf_metrics, person_paths = aggregate_visits(valid_person_frames, zones, fps, min_visit_seconds=2.0)
    attention_scores = compute_attention_score(shelf_metrics)
    most_attended_shelf = max(attention_scores, key=attention_scores.get) if attention_scores and any(attention_scores.values()) else None

    product_density = compute_product_density(product_detections, zones)
    product_engagement = compute_product_engagement(product_detections, valid_person_frames, zones, fps)
    ...
    diag_summary_text = (
        ...
        f"4. Product Model (best.pt): {'LOADED' if product_model_loaded else 'NOT PRESENT'} ({len(product_detections)} product detections)\n"
